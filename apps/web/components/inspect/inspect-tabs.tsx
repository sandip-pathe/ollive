...
import type { AgentRunSummary, EvidencePacketResponse, MetricsOverview, Trace as ApiTrace } from "@/app/lib/api";
  ...
  return (
    <ScrollArea className="flex-1 min-h-0 w-full overflow-hidden">
      <div className="mx-auto flex w-full max-w-none flex-col gap-4 px-3 py-3 sm:px-4 lg:px-5 lg:py-4">
        <OpsReview metrics={metrics} traces={traces} agentRuns={agentRuns} evidencePacket={evidencePacket} />
        ...
        <section className="rounded-lg border border-[#ddd6c6] bg-[#f6f3ea] px-4 py-4 shadow-[0_8px_24px_rgba(55,46,28,0.04)] sm:px-5">
          <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-[repeat(auto-fit,minmax(160px,1fr))]">
            {summary.map((item) => (
              <div
                key={item.label}
                className="min-w-0 rounded-lg border border-[#ddd6c6] bg-[#fbf8ef] px-4 py-3"
              >
        ...
        {selectedTrace ? (
          <section className="grid gap-4 md:grid-cols-1">
            <div className="rounded-lg border border-[#ddd6c6] bg-[#fbf9f3] shadow-[0_8px_24px_rgba(55,46,28,0.05)]">
          ...
          <section className="rounded-lg border border-dashed border-[#d5cfbf] bg-[#f9f6ee] px-4 py-8 text-sm text-[#6f685e]">
            No run selected yet. Send a chat message or ingest an AgentRun through the SDK.
          </section>
        )}
      </div>
    </ScrollArea>
...
function CompactPanel({
  ...
  return (
    <details
      className="group rounded-lg border border-[#ddd6c6] bg-[#f8f5ed] shadow-[0_8px_24px_rgba(55,46,28,0.04)]"
      open={defaultOpen}
    >
      ...
      <div className="border-t border-[#e1dacb] px-4 py-4 sm:px-5">
        <div className="space-y-2">
          {items.map((item, index) => (
            <div
              key={`${title}-${index}`}
              className="rounded-lg border border-[#ddd6c6] bg-[#fbfaf6] px-3 py-2 text-sm leading-6 text-[#3e3931] break-words whitespace-pre-wrap"
            >
              {item}
            </div>
          ))}
